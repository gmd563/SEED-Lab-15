...
float maxVelocity = .5;  // Example: 1 m/s
float maxAccel = 5;  // Max acceleration in m/s²
float posError = 0.0;
...
float maxAngularVelocity = .75;  // Example: rad/s²
float maxAngularAccel = 5;  // Max acceleration in rad/s²
...
volatile int lastDir = 1;

enum {search, move, moveToSearch};
unsigned char state = search;  
bool once = 1;
  ...
  if (receivedCount > 1) { // Ensure there's more than just the offset byte
    switch (state) {
      case search:
        ...
        desiredAngle = totalAngle + (10000 * lastDir);
        break;
      ...
      case moveToSearch:
        Serial.println("movetosearch");
        if (once == 1) {
          desiredAngle = totalAngle + (angle * (3.14159/180));
          lastDir = (angle > 0) - (angle < 0); // -1 if last angle was -, +1 if last angle was pos
          once = 0;
        }
        if (angle = 0 || detected == 1) {
          state = search;
        }
        break;
    }
...
void positionControl() {
  ...
  float deltaT = (millis() - lastTimeMs) / 1000.0;  // Convert ms to seconds

...
void setup() {
  ...
  Serial.begin(9600); // Set the baud rate fast so that we can display the results
  Serial.println("");
  Serial.println("");
...
void loop() {
  ...
  Serial.println(leftrad );
  if (millis() <= start_time_ms + 4000) {
    Serial.println(String(float(millis())/1000) + ", " + String(float(((voltage*pwm)/255))) + ", " + String(float(rightv), 4) + ", " + String(float(leftv), 4));
  }
